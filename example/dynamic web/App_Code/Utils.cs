...
namespace Common
{
    public class Utils
    {
        ...
        #region 清除HTML标记且返回相应的长度
        public static string DropHTML(string Htmlstring,string flag, int strLen)
        {
            if (flag == "置顶")
                return "<strong>" + CutString(DropHTML(Htmlstring), strLen) + "</strong>";
            else
                return CutString(DropHTML(Htmlstring), strLen);
        }
        #endregion
		...
		public static string DeCode(string s, string Key) 
		{
            ...
            try
            {
                ...
                byte[] buff = new byte[(s.Length + 1) / 2];
                ...
                for (i = 0; i < s.Length; i = i + 2)
                {
                    ...
                    t2 = s.Substring(i + 1, 1);

                    k = mstr.IndexOf(t1);
                    m = mstr.IndexOf(t2);

                    j = m / (int)System.Math.Pow(2, 3);
                    m = m - j * (int)System.Math.Pow(2, 3);

                    //重新
                    //	buff[n] = new Integer(j * mstr.Length + k).byteValue();
                    //	buff[n] = new Integer(buff[n] ^ m).byteValue();

                    buff[n] = System.Convert.ToByte(j * mstr.Length + k);
                    buff[n] = System.Convert.ToByte(buff[n] ^ m);


                    n = n + 1;
                }

                string decode = null; //把bufff转化成string 就可以了，帮我测试一下

                decode = System.Text.Encoding.UTF8.GetString(buff, 0, buff.Length); //decode应该返回"chen_m2"
