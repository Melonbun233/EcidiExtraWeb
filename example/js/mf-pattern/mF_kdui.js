myFocus.pattern.extend({//*********************kdui******************
	'mF_kdui':function(settings,$){
		...
		$focus.play(null,function(i,n,l){
			...
			$txtList.eq(l).slide({left:-settings.width},800,'');
			fx(i,l);//延迟执行部分
		});
		function fx(i,l){
			setTimeout(function(){
				$txtList.eq(i).css({display:'block',left:settings.width}).slide({left:0},1000);
				$picList.eq(i).css({display:'block',left:settings.width}).slide({left:0},function(){param.isRunning=false});
				if(i!==l) $picList.eq(l).slide({left:-settings.width},400);
				$dotList[l].className='';
